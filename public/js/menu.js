...
const generateMenu = (data, container, is_parent = true) => {
    ...
    const groupedData = data.reduce((acc, menu) => {
        const { grup_menu } = menu;
        if (!acc[grup_menu]) {
            acc[grup_menu] = [];
        }
        acc[grup_menu].push(menu);
        return acc;
    }, {});

    for (const [group, menus] of Object.entries(groupedData)) {
        let groupHeader = $('<li>', {
            class: 'menu-title',
            text: group
        });
        container.append(groupHeader.prop('outerHTML'));

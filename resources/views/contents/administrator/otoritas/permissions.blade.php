            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('otoritas.submit.permission') }}" method="post">
                        ...
                        <div class="row">
                            @foreach ($menus as $group => $menusInGroup)
                                                    ...
                                                    <div class="col-1">
                                                        <div class="custom-control custom-checkbox  mr-1">
                                                            <input type="checkbox" value="1"
                                                                ...
                                                                {{ in_array($action->id, $menu->actions) ? 'checked' : '' }}>
                                                            <label class="custom-control-label"
                                                                for="{{ $menu->id . '_' . $role->id . '_' . $action->id }}">{{ Str::ucfirst($action->name) }}</label>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            @else
                                                ...
                                                <div class="col-1 mb-3">
                                                    <div class="custom-control custom-checkbox  mr-1">
                                                        <input type="checkbox" value="1" class="custom-control-input"
                                                            ...
                                                            {{ in_array($first_action->id, $menu->actions) ? 'checked' : '' }}>
                                                        <label class="custom-control-label"
                                                            for="{{ $menu->id . '_' . $role->id . '_' . $first_action->id }}">{{ Str::ucfirst($first_action->name) }}</label>
                                                    </div>
                                                </div>
                                                                    ...
                                                                    <div class="col-1">
                                                                        <div class="custom-control custom-checkbox  mr-1">
                                                                            <input type="checkbox" value="1"
                                                                                ...
                                                                                {{ in_array($action->id, $child->actions) ? 'checked' : '' }}>
                                                                            <label class="custom-control-label"
                                                                                for="{{ $child->id . '_' . $role->id . '_' . $action->id }}">{{ Str::ucfirst($action->name) }}</label>
                                                                        </div>
                                                                    </div>
                                                                @endforeach
                                                            </div>
